...
function out = solve_part2(NUMBER_OF_SATELLITES, NUMBER_OF_STATIONS, RANDOM_VELOCITIES, INVERSE_VELOCITIES_SATEL,...
    ...
    record = false; % true -> creates a matlab video of the 3D orbits. SHOW_TOPOLOGY must be true!
    
    %%%% Create matlab video:
    if record
        writerObj = VideoWriter('C:\Users\User\Desktop\3D sphere rotation.avi'); % Name it.
...
%               ", total epochs:"+string(STOP_AT_TIME));
            ...
            axis equal
            axis off
            view(120,0)% setting azemuth and elevation angles of camera for nicer visualization 
        level = 70;
        axis([-earth_radius-level earth_radius+level -earth_radius-level earth_radius+level -earth_radius-level earth_radius+level]); %setting up figure size
    end       
...
%        num_station_links = get_nstat_links(xij, NUMBER_OF_SATELLITES);   % number of satellite-station undirectional links
       ...
       if SHOW_TOPOLOGY
           ...
           pause(0.001) %WARNING: all 'delete' (for graphic objects) functions must be after the 'pause' function 
