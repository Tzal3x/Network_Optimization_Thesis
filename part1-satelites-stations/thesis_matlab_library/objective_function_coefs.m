function out = objective_function_coefs(num_flows, num_of_nodes, total_epochs, NUMBER_OF_SATELLITES)
    coefficients = null(1);
    for epoch = 1:total_epochs      
        coefficients = [ coefficients, ones(1, num_flows)*(10^(-3))];% flows
        coefficients = [ coefficients, ones(1, NUMBER_OF_SATELLITES)]; % divergencies of satellites
